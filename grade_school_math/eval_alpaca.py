    ...
    for batch in tqdm(eval_dataloader):
        with torch.no_grad():
            try:
                ...
                print(batch_output.sequences.shape, batch_output.sequences)
            except RuntimeError as e:
                print(e)
                raise e
        outputs_string = tokenizer.batch_decode(batch_output.sequences, skip_special_tokens=True)
        print(f"Batch output sequences shape: {batch_output.sequences.shape}, sequences: {batch_output.sequences}")
        print(f"Initial pred_ans_list length: {len(pred_ans_list)}, gold_ans_list length: {len(gold_ans_list)}")
...
def fsdp_main():
    ...
    #         print("model loaded")
    # print(f"debug333 rand: {rank}", model.get_input_embeddings().weight)
    ...
    # }
    # print(fsdp_config)
    # fsdp = FullyShardedDataParallel(model, **fsdp_config)
    # print(f"debug444 rand: {rank}", fsdp.get_input_embeddings().weight)
    # fsdp.eval()
