...
class GSMDataset(torch.utils.data.Dataset):
    ...
    def process_example(self, example):
        question_tokens = self.tokenizer.encode(example["question"], add_special_tokens=False)
        answer_tokens = self.tokenizer.encode(example["answer"], add_special_tokens=False)
        input_ids = question_tokens + [self.sep_token] + answer_tokens
        label_ids = [-100] * len(question_tokens) + [
            -100] + answer_tokens  # -100 is commonly used to ignore token in loss computation
        return {
            "input_ids": input_ids,
            "label_ids": label_ids,
            "q_ids": question_tokens
        }
    ...
    def __getitem__(self, idx):
        item = self.processed_examples[idx]
        ...
        attention_mask = [1] * len(input_ids) + [0] * input_padding_length
        q_attention_mask = [1] * len(q_ids) + [0] * q_padding_length
        
        input_ids += [self.tokenizer.pad_token_id] * input_padding_length
        label_ids += [-100] * input_padding_length
